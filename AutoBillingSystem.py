...
root.title("Auto Billing Management System")
...
def CostOfOrder():
    ...
    CarpetCost ='Pesos',str('%.2f' %(CarpetPrice * 1200))
    CostofCarpets.set(CarpetCost)

    BlindsCost ='Pesos',str('%.2f' %(BlindsPrice * 2000))
    CostofBlinds.set(BlindsCost)

    FabricCost ='Pesos',str('%.2f' %(FabricPrice * 750))
    CostofFabric.set(FabricCost)

    Delivery ='Pesos',str('%.2f' %(DeliveryCost * 500))
    CostofDelivery.set(Delivery)

    Toc ='Pesos',str('%.2f' %((CarpetPrice * 1200)+(BlindsPrice * 2000)+(FabricPrice * 750)+(DeliveryCost * 500)))
    SubTotal.set(Toc)
    Tax ='Pesos',str('%.2f' %(((CarpetPrice * 1200)+(BlindsPrice * 2000)+(FabricPrice * 750)+(DeliveryCost * 500))*0.05))
    PaidTax.set(Tax)
    ...
    CostofItems='Pesos',str('%.2f' %(TaxPay + Cost))
    TotalCost.set(CostofItems)

    x=random.randint(10034,699812)
    randomRef = str(x)
    PaymentRef.set('1chu' + randomRef)
...
def iExit():
     qExit=messagebox.askyesno('Billing System', 'Do you want to exit the systems')
     if qExit>0:
         root.destroy()
         return
